...
{
  sample(length(regions), size = nsim, prob = regions, replace = TRUE)
}
...
prior_post_plot <- function (d) 
{
  ...
  ggplot(D1, aes(Model, Probability)) + 
    geom_segment(aes(xend = Model, yend = 0, colour = Type), 
                 size = Size, lineend = "butt") + 
    facet_wrap(~Type, ncol = 1)
}

# beta_area
beta_area <- function (lo, hi, shape_par, Color = "orange") 
{
  x <- seq(0, 1, by = 0.001)
...
# beta_quantile
beta_quantile <- function (prob, shape_par, Color = "orange") 
{
    x <- seq(0, 1, by = 0.001)
    dx <- dbeta(x, shape_par[1], shape_par[2])
    p <- ggplot(data.frame(x = x, dx = dx), aes(x, dx)) + geom_line()
    HI <- qbeta(prob, shape_par[1], shape_par[2])
    x0 <- seq(0, HI, by = 0.001)
    y0 <- dbeta(x0, shape_par[1], shape_par[2])
    xx <- c(0, x0, HI, 0)
    yy <- c(0, y0, 0, 0)
    message <- paste("P(0 < P < ", round(HI, 3), ") =", prob)
    other_text <- paste("Beta(", shape_par[1], ", ", shape_par[2], 
        ")", sep = "")
    x_text <- ifelse(shape_par[1] > shape_par[2], 0.15, 0.85)
    y_text <- 0.7 * max(dx)
...
# beta_interval
beta_interval <- function (prob, shape_par, Color = "orange") 
{
  x <- seq(0, 1, by = 0.001)
  dx <- dbeta(x, shape_par[1], shape_par[2])
  p <- ggplot(data.frame(x = x, dx = dx), aes(x, dx)) + geom_line()
...
# beta_prior_post
beta_prior_post <- function (prior_shapes, post_shapes, label_1 = "Prior", label_2 = "Posterior") 
{
  TH <- theme(plot.title = element_text(colour = "blue", size = 18, 
                                        hjust = 0.5, vjust = 0.8, angle = 0))
  x <- NULL
  ggplot(data.frame(x = c(0, 1)), aes(x)) + 
    stat_function(fun = dbeta, geom = "line", aes(linetype = label_1), 
                  size = 1.5, args = list(shape1 = prior_shapes[1], shape2 = prior_shapes[2])) + 
    stat_function(fun = dbeta, geom = "line", aes(linetype = label_2), 
                  size = 1.5, args = list(shape1 = post_shapes[1], shape2 = post_shapes[2])) + 
    scale_colour_manual(values = c("red", "blue")) + labs(colour = "Type") + xlab("P") + ylab("Density")
}
...
# prob_plot
prob_plot <- function (d, Color = "red", Size = 1.5) 
{
  N <- dim(d)[1]
  TH <- theme(plot.title = element_text(colour = "blue", size = 18, 
                                        hjust = 0.5, vjust = 0.8, angle = 0))
  p <- ggplot(d, aes_string(colnames(d)[1], colnames(d)[2])) + 
    geom_segment(aes_string(xend = colnames(d)[1], yend = 0), 
                 size = Size, lineend = "butt", color = Color) + TH
  p
}
...
many_normal_plots <- function (list_normal_par) 
{
  ...
  p <- ggplot(df, aes(y, f, group = Model)) + geom_line(aes(color=Model), size = 1.5)
  if (N == 2) {
...
# normal_draw
normal_draw <- function (normal_pars, Color = "red") 
{
  normal_pars <- unlist(normal_pars)
  TH <- theme(plot.title = element_text(
    colour = "blue", size = 18, hjust = 0.5, vjust = 0.8, angle = 0))
  Title <- paste("Normal(", normal_pars[1], ",", normal_pars[2], ") Curve")
  x_lo <- normal_pars[1] - 4 * normal_pars[2]
  x_hi <- normal_pars[1] + 4 * normal_pars[2]
  x <- NULL
  ggplot(data.frame(x = c(x_lo, x_hi)), aes(x)) + 
    stat_function(fun = dnorm, geom = "line", color = Color, size = 2.5, 
                  args = list(mean = normal_pars[1], sd = normal_pars[2])) + 
    ggtitle(Title) + TH + ylab("Density")
}

# normal_area
normal_area <- function (lo, hi, normal_pars, Color = "orange") 
{
  normal_pars <- unlist(normal_pars)
  x_lo <- normal_pars[1] - 4 * normal_pars[2]
  x_hi <- normal_pars[1] + 4 * normal_pars[2]
  ...
  message <- paste("P(", round(lo, 3), "< M < ", round(hi, 
                                                       3), ") =", round(normprob, 3))
  other_text <- paste("Normal(", normal_pars[1], ", ", normal_pars[2], 
                      ")", sep = "")
  ...
  TH <- theme(plot.title = element_text(colour = "blue", size = 18, 
                                        hjust = 0.5, vjust = 0.8, angle = 0))
  p + geom_polygon(data = data.frame(xx, yy), aes(xx, yy), 
                   fill = Color) + ggtitle(message) + xlab("M") + ylab("Density") + 
    TH + annotate("text", x = x_text, y = y_text, label = other_text, 
                  size = 6, color = "blue")
}

# normal_quantile
normal_quantile <- function (prob, normal_pars, Color = "orange") 
{
  text = TRUE
  normal_pars <- unlist(normal_pars)
  x_lo <- normal_pars[1] - 4 * normal_pars[2]
  ...
  message <- paste("P( M < ", round(hi, 2), ") =", round(normprob, 
                                                         3))
  other_text <- paste("Normal(", normal_pars[1], ", ", normal_pars[2], 
                      ")", sep = "")
  x_text <- normal_pars[1] - 2.5 * normal_pars[2]
  ...
  TH <- theme(plot.title = element_text(colour = "blue", size = 18, 
                                        hjust = 0.5, vjust = 0.8, angle = 0))
  p <- p + geom_polygon(data = data.frame(xx, yy), aes(xx, yy), fill = Color) + 
    ggtitle(message) + xlab("M") + ylab("Density") + 
    TH
  if (text == TRUE) 
    p <- p + annotate("text", x = x_text, y = y_text, label = other_text, 
                      size = 6, color = "blue")
  p
}

# normal_interval
normal_interval <- function (prob, normal_pars, Color = "orange") 
{
  normal_pars <- unlist(normal_pars)
  x_lo <- normal_pars[1] - 4 * normal_pars[2]
  x_hi <- normal_pars[1] + 4 * normal_pars[2]
...
# testing_prior
testing_prior <- function (lo = 0.1, hi = 0.9, n_values = 9, pequal = 0.5, uniform = FALSE) 
{
  p1 <- seq(lo, hi, length = n_values)
  p2 <- p1
  n_diagonal <- n_values
  n_off_diag <- n_values^2 - n_values
  prior <- matrix(0, n_values, n_values) + (1 - pequal)/n_off_diag
...
draw_two_p <- function (prob_matrix, ...) 
{
  ...
  Type <- ifelse(P1 == P2, "P1 = P2", ifelse(P1 < P2, "P1 < P2", 
                                             "P1 > P2"))
...
two_p_summarize <- function (prob_matrix) 
{
  ...
  df <- summarize(group_by(df, diff21), Prob = sum(PROB), .groups = 'drop_last')
